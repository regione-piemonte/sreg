...
@Component({
    selector: 'app-modelloF',
    templateUrl: './modelloF.component.html',
    styleUrls: ['./modelloF.component.css']
})
export class ModelloFComponent implements OnInit {
  ...
  datiEnte: DatiEnteGreg;
    ...
    calcTableValue() {     
       ...
       this.totTotCOpO=this.parsingInt(this.totaleTotCO)+this.parsingInt(this.totaleRO[5]);
        ...
        this.totaleMonteOreDip = this.roundTo(this.parsingInt(this.totaleTotCP)==0 ? 0 : this.parsingInt(this.totaleTotCOpO)/this.parsingInt(this.totaleTotCP),1).toString();
        this.totaleMonteOreNonDip = this.roundTo(this.parsingInt(this.totaleRP[4])==0 ? 0 : this.parsingInt(this.totaleRO[6])/this.parsingInt(this.totaleRP[4]),1).toString();
...
//                        if ((this.client.ruolo==ROLES.OPERATORE_REGIONALE || this.client.ruolo==ROLES.SUPERUSER)
    ...
    esportaexcel() {
		...
		if (this.ischangedvar) {
            ...
            let monteOreSettInterno: string[] = [];
            let monteOreSettEsterno: string[] = [];
            for(let o of this.monteOreDip){
                if(o!=''){
                    monteOreSettInterno.push(parseFloat(o).toString());
                }else{
                    monteOreSettInterno.push('');
                }
            }
            for(let o of this.monteOreNonDip){
                if(o!=''){
                    monteOreSettEsterno.push(parseFloat(o).toString());
                }else{
                    monteOreSettEsterno.push('');
                }
            }
            esporta.monteOreSettInterno = monteOreSettInterno;
            esporta.monteOreSettEsterno = monteOreSettEsterno;
            esporta.totaleMonteOreSettInterno = parseFloat(this.totaleMonteOreDip).toString();
            esporta.totaleMonteOreSettEsterno = parseFloat(this.totaleMonteOreNonDip).toString();
