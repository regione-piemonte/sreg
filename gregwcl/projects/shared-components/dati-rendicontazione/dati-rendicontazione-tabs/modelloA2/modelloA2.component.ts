...
@Component({
  selector: 'app-modelloA2',
  templateUrl: './modelloA2.component.html',
  styleUrls: ['./modelloA2.component.css'],
  animations: [
...
export class ModelloA2Component implements OnInit {
  ...
  trasfEnte: string;
  trasfEnteSwap: string;
  trasfComune: string;
  trasfComuneSwap: string;
  totaleEnte;
  totaleComune;
  // espansa:boolean;
  ...
  transformAmountEnte(from: string) {
    if (from == 'ente') {
      ...
      let risultato_swap = this.trasfEnteSwap.replace(/\./g, '');
      this.trasfEnteSwap = risultato_swap.replace(/,/g, '.');
      this.trasfEnte = this.trasfEnteSwap ? parseFloat(this.trasfEnteSwap).toFixed(2) : this.trasfEnteSwap;
      const floatToUse : number = parseFloat(this.trasfEnteSwap);
      if (isNaN(floatToUse)) {
        this.trasfEnteSwap = null;
      } else {
        this.trasfEnteSwap = floatToUse.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      }
    } else if (from == 'comune') {
      let risultato_swap = this.trasfComuneSwap.replace(/\./g, '');
      this.trasfComuneSwap = risultato_swap.replace(/,/g, '.');
      this.trasfComune = this.trasfComuneSwap ? parseFloat(this.trasfComuneSwap).toFixed(2) : this.trasfComuneSwap;
      const floatToUse : number = parseFloat(this.trasfComuneSwap);
      if (isNaN(floatToUse)) {
        this.trasfComuneSwap = null;
      } else {
        this.trasfComuneSwap = floatToUse.toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
      }
    }
  }
