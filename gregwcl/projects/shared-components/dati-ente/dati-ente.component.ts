...
import { DettaglioPrestazioneComponent } from './dettaglio-prestazione/dettaglio-prestazione.component';
...
@Component({
  ...
  styleUrls: ['./dati-ente.component.css'],
  animations: [
...
export class DatiEnteComponent implements OnInit {
  ...
  columnsModelliAssociati: Array<string> = ['modello', 'obbligatorio', 'azioni'];
  ...
  successoEliminazione: string;
  messaggioPopupAzione: string;
  fondoDaAssegnareInitial: Fondi[];
  totale: number = 0;
  ...
  ngOnInit() {
    ...
    forkJoin({
      ...
      prestPopUp: this.client.getMsgInformativi(SECTION.PROBLEMAPRESTAZIONE),
      ...
      regole: this.client.getRegole(),
      messaggioPopupElimina: this.client.getMsgInformativi(SECTION.MESSAGGIOELIMINAFONDO),
      successoEliminazione: this.client.getMsgInformativi(SECTION.CONFERMAELIMINAZIONEFONDI),
      messaggioPopupAzione: this.client.getMsgInformativi(SECTION.MESSAGGIOELIMINAVINCOLO),
      messaggioPopupCalcolo: this.client.getMsgInformativi(SECTION.MSGCALCOLO),
      fondiEnte: this.client.getFondiEnteModelloAlZero(this.rendicontazione.idRendicontazioneEnte),
      tooltipFondiAlZero: this.client.getMsgInformativi(SECTION.ALZEROFONDI),
    })
      .subscribe(({ msgErrorPrest, msgErrorDoc, msgErrorComuneAlreadyPresent, msgErrorPrestAlreadyPresent, msgErrorUploadAllegati,
        ...
        tipoEnte,
        docAssociati, prestRes, prestSemires, msgTab, errPippi, modelli, modelliassociati, obbligatorio, msgErrorDeleteModelliVal,
        prestPopUp, utenzeFnps, fondi, fondoDaAssegnare, regole, messaggioPopupElimina, successoEliminazione, messaggioPopupAzione, messaggioPopupCalcolo,
        fondiEnte, tooltipFondiAlZero
      }) => {
        this.tooltipFondiAlZero = tooltipFondiAlZero[0].testoMsgInformativo;
        ...
        this.messaggioPopupAzione = messaggioPopupAzione[0].testoMsgInformativo;
        this.messaggioPopupCalcolo = messaggioPopupCalcolo[0].testoMsgInformativo;
        this.successoEliminazione = successoEliminazione[0].testoMsgInformativo;
        ...
        this.popoverPrestBody = prestPopUp[0].testoMsgInformativo;
        ...
        // Fondo ente per allontanamento zero
        this.fondiEnte = fondiEnte;
        if(this.fondiEnte.fondoRegionale) {
          // this.changeFondoEnte(this.fondiEnte.fondoRegionale.toString());
          this.fondoEnte = this.transform(parseFloat(this.fondiEnte.fondoRegionale.toString().replace(',', '.')));
        }
        if(this.fondiEnte.quotaAllontanamentoZero) {
          this.fondoAlZero = this.transform(parseFloat(this.fondiEnte.quotaAllontanamentoZero.toString().replace(',', '.')));
          // this.changeFondoAlZero(this.fondiEnte.quotaAllontanamentoZero.toString());
        } else {
          this.fondiEnte.quotaAllontanamentoZero = 0;
          this.fondoAlZero = this.transform(parseFloat(this.fondiEnte.quotaAllontanamentoZero.toString().replace(',', '.')));
        }
        ...
        this.fondoDaAssegnare = JSON.parse(JSON.stringify(fondoDaAssegnare));
        this.fondoDaAssegnareInitial = JSON.parse(JSON.stringify(fondoDaAssegnare));
        ...
        this.fondi = fondi;
        for (let f of fondoDaAssegnare) {
          this.fondoDaAssegnare = this.fondoDaAssegnare.filter(u => !this.fondi.some(c => c.idFondo == u.idFondo));
        }
        this.fondoDaAssegnare = this.fondoDaAssegnare.filter(u => this.fondoFnps.idFondo != u.idFondo);
        this.fondoDaAssegnareInitial = this.fondoDaAssegnareInitial.filter(u => this.fondoFnps.idFondo != u.idFondo);
        ...
        this.regolaDaAssegnare = JSON.parse(JSON.stringify(regole));
        this.idSelectedRegola = this.regolaDaAssegnare[0].idRegola;
        ...
        this.regolaDaAssegnareAzioni = JSON.parse(JSON.stringify(regole));
        this.idSelectedRegolaAzioni = this.regolaDaAssegnareAzioni[0].idRegola;
    ...
    if (this.denominazioneEnte.rendicontazione) {
      if (this.client.azioni.get("DatiEnteAllegati")[0]) {
        ...
        if ((this.denominazioneEnte.rendicontazione.codStatoRendicontazione == RENDICONTAZIONE_STATUS.IN_RIESAME_I ||
          ...
          this.denominazioneEnte.rendicontazione.codStatoRendicontazione == RENDICONTAZIONE_STATUS.CONCLUSA ||
  ...
  changeFondoEnte(valore: any) {
    if (valore == "" || valore == null) {
      ...
      if (valore.indexOf('.') !== -1) {
        if (valore.indexOf(',') !== -1 && (valore.slice(valore.indexOf(',') + 1, valore.length)).length != 2)
          valore = [valore, '0'].join('');
          this.fondoEnte = valore;
      }
      else this.fondoEnte = this.transform(parseFloat(valore.toString().replace(',', '.')));
    }
  }
  changeFondoAlZero(valore: any) {
    if (valore == "" || valore == null) {
      this.fondoAlZero = null;
    }
    else {
      if (valore.indexOf('.') !== -1) {
        if (valore.indexOf(',') !== -1 && (valore.slice(valore.indexOf(',') + 1, valore.length)).length != 2)
          valore = [valore, '0'].join('');
        this.fondoAlZero = valore;
      }
      else this.fondoAlZero = this.transform(parseFloat(valore.toString().replace(',', '.')));
    }
  }
  ...
  salvaModifiche() {
    if ((this.client.azioni.get("CronologiaRegionale")[1] && !this.client.azioni.get("CronologiaRegionale")[0])
      ...
      this.datiEnteToSave.fondi = [];
      // if (this.fnps.valore != null) {
        let fondo = JSON.parse(JSON.stringify(this.fnps))
        fondo.valore = this.parsingFloat(fondo.valore)
        this.datiEnteToSave.fondi.push(fondo);
      // } 
      // else {
      //   this.fnps.valore = "0,00";
      //   let fondo = JSON.parse(JSON.stringify(this.fnps))
      //   fondo.valore = this.parsingFloat(fondo.valore)
      //   this.datiEnteToSave.fondi.push(fondo);
      // }
      for (let q of this.quote) {
        let fondo = JSON.parse(JSON.stringify(q))
        fondo.valore = this.parsingFloat(fondo.valore)
        this.datiEnteToSave.fondi.push(fondo);
      }
      for (let q of this.azioniSistema) {
  ...
  controlloFineValidita(prest: PrestazioneAssociataGreg) {
    if ((prest.annoFineValidita != null && prest.annoFineValidita >= this.rendicontazione.annoEsercizio) || prest.annoFineValidita == null) {
      return true;
    }
    return false;
  }

  popUpPrest(prest: PrestazioneAssociataGreg) {
    let fineV = new Date(prest.dataFineValidita);
    let data = fineV.getDate() + "/" + (fineV.getMonth() + 1) + "/" + fineV.getFullYear();
    return this.popoverPrestBody.replace("PRESTAZIONE", prest.codicePrestazione + " - " + prest.desPrestazione).replace("DATA", data);
  }

  aggiungiAltreQuote() {
    this.fondoSelezionato.esistente = false;
    this.quote.push(JSON.parse(JSON.stringify(this.fondoSelezionato)));
    this.altreQuoteDaAssegnare = this.altreQuoteDaAssegnare.filter(f => f.idFondo !== this.fondoSelezionato.idFondo);
    this.calcTableValue();
    this.fondoSelezionato.idFondo = 0;
    this.idSelectedAltreQuote = 0;
    this.fondoSelezionato.valore = null;
    this.fondoSelezionato.idRegola = this.regolaDaAssegnare.find(f => f.funzioneRegola == '+').idRegola;
    this.idSelectedRegola = this.regolaDaAssegnare.find(f => f.funzioneRegola == '+').idRegola;
  }

  disableAltreQuote() {
    if (this.quote.some(f => f.codFondo == this.fondoSelezionato.codFondo)) {
      return true;
    } else if (this.fondoSelezionato.idFondo == 0 || this.idSelectedAltreQuote == 0) {
      return true;
    } else if (this.fondoSelezionato.valore == null) {
      return true;
    }
    return false;
  }
  ...
  aggiungiAzioni() {
    this.azioneSelezionato.esistente = false;
    this.azioniSistema.push(JSON.parse(JSON.stringify(this.azioneSelezionato)));
    this.azioniDaAssegnare = this.azioniDaAssegnare.filter(f => f.idFondo !== this.azioneSelezionato.idFondo);
    this.calcTableValue();
    this.azioneSelezionato.idFondo = 0;
    this.idSelectedAzioni = 0;
    this.azioneSelezionato.valore = null;
    this.azioneSelezionato.idRegola = this.regolaDaAssegnareAzioni.find(f => f.funzioneRegola == '+').idRegola;
    this.idSelectedRegolaAzioni = this.regolaDaAssegnareAzioni.find(f => f.funzioneRegola == '+').idRegola;
  }

  disableAzioni() {
    if (this.azioniSistema.some(f => f.codFondo == this.azioneSelezionato.codFondo)) {
      return true;
    } else if (this.azioneSelezionato.idFondo == 0 || this.idSelectedAzioni == 0) {
      return true;
    } else if (this.azioneSelezionato.valore == null) {
      return true;
    }
    return false;
  }
  ...
  onSelectionFondoChanged(value: number) {
    let fondo = this.altreQuoteDaAssegnare.find(u => u.idFondo == value);
    if (fondo) {
      this.fondoSelezionato.idFondo = fondo.idFondo;
      this.fondoSelezionato.codFondo = fondo.codFondo;
      this.fondoSelezionato.descFondo = fondo.descFondo;
      this.fondoSelezionato.codTipologiaFondo = fondo.codTipologiaFondo;
      this.fondoSelezionato.leps = fondo.leps;
    } else {
      this.fondoSelezionato.idFondo = 0;
    }

    let regola = this.regolaDaAssegnare.find(r => r.idRegola == this.idSelectedRegola);
    if (regola != null) {
      this.fondoSelezionato.idRegola = regola.idRegola;
      this.fondoSelezionato.codRegola = regola.codRegola;
      this.fondoSelezionato.descRegola = regola.descRegola;
      this.fondoSelezionato.funzioneRegola = regola.funzioneRegola;
    } else {
      this.fondoSelezionato.idRegola = 0;
      this.fondoSelezionato.funzioneRegola = " ";
    }
  }

  onSelectionRegolaChanged(value: number) {
    let regola = this.regolaDaAssegnare.find(u => u.idRegola == value);
    if (regola != null) {
      this.fondoSelezionato.idRegola = regola.idRegola;
      this.fondoSelezionato.codRegola = regola.codRegola;
      this.fondoSelezionato.descRegola = regola.descRegola;
  ...
  onSelectionAzioniChanged(value: number) {
    let azione = this.azioniDaAssegnare.find(u => u.idFondo == value);
    if (azione) {
      this.azioneSelezionato.idFondo = azione.idFondo;
      this.azioneSelezionato.codFondo = azione.codFondo;
      this.azioneSelezionato.descFondo = azione.descFondo;
      this.azioneSelezionato.codTipologiaFondo = azione.codTipologiaFondo;
      this.azioneSelezionato.leps = azione.leps;
    } else {
      this.azioneSelezionato.idFondo = 0;
    }
    let regola = this.regolaDaAssegnareAzioni.find(r => r.idRegola == this.idSelectedRegolaAzioni);
    if (regola != null) {
      this.azioneSelezionato.idRegola = regola.idRegola;
      this.azioneSelezionato.codRegola = regola.codRegola;
      this.azioneSelezionato.descRegola = regola.descRegola;
      this.azioneSelezionato.funzioneRegola = regola.funzioneRegola;
    } else {
      this.azioneSelezionato.idRegola = 0;
      this.azioneSelezionato.funzioneRegola = " ";
    }
  }

  onSelectionRegolaAzioniChanged(value: number) {
    let regola = this.regolaDaAssegnareAzioni.find(u => u.idRegola == value);
    if (regola != null) {
      this.azioneSelezionato.idRegola = regola.idRegola;
      this.azioneSelezionato.codRegola = regola.codRegola;
      this.azioneSelezionato.descRegola = regola.descRegola;
  ...
  eliminaQuota(quota: Fondi, i: number) {
    let successo = this.successoEliminazione.replace("FONDO", quota.descFondo);
    quota.valore = this.parsingFloat(quota.valore);
    if (quota.esistente) {
      let messaggio = this.messaggioPopupElimina.replace("FONDO", quota.descFondo);
      ...
      dialogRef.afterClosed().subscribe(r => {
        if (r) {
          this.client.spinEmitter.emit(true);
          this.quote = this.quote.filter((f, index) => index != i);
          this.quote.sort((a, b) =>
            this.sortFondo(a, b));
          this.altreQuoteDaAssegnare.push(this.fondoDaAssegnareInitial.filter(f => quota.idFondo === f.idFondo)[0]);
          this.altreQuoteDaAssegnare.sort((a, b) =>
            this.sortFondo(a, b));
          this.client.eliminaFondo(quota).subscribe(r => {
            if (r) {
              this.toastService.showSuccess({ text: successo });
              this.client.spinEmitter.emit(false);
            }
          },
            err => {
  ...
  eliminaAzione(azione: Fondi, i: number) {
    let successo = this.successoEliminazione.replace("FONDO", azione.descFondo);
    azione.valore = this.parsingFloat(azione.valore);
    if (azione.esistente) {
      let messaggio = this.messaggioPopupAzione.replace("VINCOLO", azione.descFondo);
      const dialogRef = this.dialog.open(CancPopupComponent, {
        width: '650px',
        disableClose: true,
        autoFocus: true,
      ...
      dialogRef.afterClosed().subscribe(r => {
        if (r) {
          this.client.spinEmitter.emit(true);
          this.azioniSistema = this.azioniSistema.filter((f, index) => index != i);
          this.azioniSistema.sort((a, b) =>
            this.sortFondo(a, b));
          this.azioniDaAssegnare.push(this.fondoDaAssegnareInitial.filter(f => azione.idFondo === f.idFondo)[0]);
          this.azioniDaAssegnare.sort((a, b) =>
            this.sortFondo(a, b));
          this.client.eliminaFondo(azione).subscribe(r => {
            if (r) {
              this.toastService.showSuccess({ text: successo });
              this.client.spinEmitter.emit(false);
            }
