...
@Component({
  selector: 'app-modello-b1-archivio',
  templateUrl: './modello-b1-archivio.component.html',
  styleUrls: ['./modello-b1-archivio.component.css'],
  animations: [
...
export class ModelloB1ArchivioComponent implements OnInit {
    ...
    if (macroaggregati && macroaggregati.length>0) {
      macroaggregati.forEach(element => {
        if (element.codice=="1VB") { //REDDITO DAL LAVORO DIPENDENTE
          reddito_da_lavoro_compilato = element.valore != undefined && element.valore!="" && this.parsingFloat(element.valore) != null;
