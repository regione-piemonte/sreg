...
/**
 * Il filtro agisce su tutte le richieste in input. Serve a prevenire attacchi di tipo XSRF.
...
//	}
	...
	//protected static final Logger LOG = Logger.getLogger(XSRFProtectionFilter.class);
	//final static Logger LOG = Logger.getLogger(Constants.COMPONENT_NAME);
	private final static Logger log = Logger.getLogger(XSRFProtectionFilter.class.getName());
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		if (!isDisabled) {
			...
			if (!isRestRequest(hreq)) {
				log.info("[XSRFProtectionFilter::dofilter() - request is NOT a REST request");
				if (!isXSRFSessionAlive(session)) {
					log.info("[XSRFProtectionFilter::dofilter() - XSRF token NOT already initialized: creating new one]");
					try {
						String newToken = createNewXSRFToken(session);
						addXSRFCookie(hresp, newToken);
						log.info("[XSRFProtectionFilter::dofilter() - new XSRF token created and added to session and response]");
					} catch (NoSuchAlgorithmException e) {
						throw new ServletException(e);
					}
				} else {
					log.info("[XSRFProtectionFilter::dofilter() - XSRF token already initialized: nothing to do]");
				}
			} else { // rest request: token must be valid
				log.info("[XSRFProtectionFilter::dofilter() - request is a REST request]");
				if (isXSRFSessionAlive(session)) {
					log.info("[XSRFProtectionFilter::dofilter() - XSRF token already initialized => must check request]");
					if (validXSRFCookieAndHeader(hreq, session)) {
						log.info("[XSRFProtectionFilter::dofilter() - XSRF cookie&header matches in request => OK]");
						//OK
					} else {
						// ERRORE!!!
						log.info("[XSRFProtectionFilter::dofilter() - Invalid XSRF Header in request]");
